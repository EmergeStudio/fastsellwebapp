...
echo open_div('middle').open_div('whiteBack coolScreen');
	...
	echo open_div('controlBar plain');
		...
		echo open_div('shifter');
			...
			// Slide nav 2
			echo full_div('Choose File And Definition', 'shifterBlock');

			// Slide nav 3
			echo full_div('Check Your Data', 'shifterBlock');

			// Slide nav 4
			echo full_div('Upload Items', 'shifterBlock shifterEnd');
	...
	echo open_div('shifterPane shifterPane_2');
		...
		echo form_open_multipart('ajax_handler_products/check_products_upload', 'class="frmCheckItemUpload"');
			...
			// Content
			echo '<p>Upload your Excel file that will be used to import all your items.</p>';
			...
			echo open_div('inset');
				...
				if($item_defs['error'] == FALSE)
				{
					...
					// Content
					echo '<p>You can only upload a master file against one item definition at a time.  Please select your definition below in order to proceed.</p>';
				...
				else
				{
					...
					// Content
					echo '<p>In order to proceed you need an item definition to upload to.</p>';
					echo make_button('Add Item Definition', '', 'item_catalogue/definitions');
				}
	...
	echo open_div('shifterPane shifterPane_4');
		...
		// Heading
		echo heading('Upload Your Items', 2);

		// Content
		echo '<p>If you have checked your data and happy with the changes that you have made then submit the items for uploading by clicking the <b>"Upload Items"</b> button in the bottom right. Alternatively go back a step to modify your data again or go further back to select a different master file to upload.</p>';
		echo '<p>Any item that already exists will be updated, any item that is new will be created and any item that does not check out will not be created at all.</p>';

		// Check data loader
		echo full_div('Uploading Your Items Now', 'loader displayNone');
	...
	echo open_div('shifterNav');
		...
		echo make_button('Upload Items', 'btnComplete', '', 'right', '', FALSE);
